...
class survey_html_create
{
    ...
    function __construct()
    {
        ...
        $this->con = $database->login_database("register");
    }
    ...
    public function get_post_info()
    {
        if (isset($_POST["name"]) && isset($_POST["id"]) && isset($_POST["answer"])&&isset($_POST["to"])) {
            ...
            echo json_encode($this->con->query("LOCK TABLE $survey_table_name WRITE"));
            ...
            */
            ...
            foreach ($sql_arr as $key => $val) {
                echo json_encode($this->con->query($val));
            }
            ...
            echo json_encode($this->con->query("UNLOCK TABLES"));
            //echo json_encode($result_arr);
        } else {
            $arr = array(
                "code" => "403",
    ...
    public function template_page_create($arr,$writer,$title)
    {
        if(($this->create_table($title."_".$writer,count($arr)))==true){
            $sql="insert into survey(writer,survey_name,data,time,flag)values('$writer','$title.'_'.$writer','date('Y-m-d')','on_survey')";
            $this->con->query($sql);
        }
        $index = 1;
        $html = "";
