...
//         }
     ...
     public function ip_limit($max_request){
         $cache_data=cache_::lock_get_cache($this->request_->get_ip_address());
         if($cache_data==null){
             cache_::lock_set_cache($this->request_->get_ip_address(),["time"=>date("Y-m-d H:i"),"num"=>1,"now_request"=>1],86400);
         }
         else{
             ...
             if($cache_data["time"]==date("Y-m-d H:i")){
                 if($cache_data["num"]>5){
                     ...
                     cache_::lock_set_cache($this->request_->get_ip_address(),$cache_data,86400);
                 }
             }
             else{
     ...
     public function leaky_bucket($max_capacity){
         $leaky_bucket_cache=cache_::lock_get_cache($this->request_->get_url());
         if($leaky_bucket_cache==null){
             cache_::set_cache($this->request_->get_url(),["now_capacity"=>1],"forever");
             return;
         }
         else{
