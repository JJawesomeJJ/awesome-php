...
class cache
{
    ...
    public  function set_cache($key,$value,$expire,$is_serialize=false,$is_timestamp=false){
        ...
        if ($this->driver=="file")
        {
            if(!$is_serialize) {
                ...
                $this->file->write_file($this->path . md5($key) . ".txt", serialize(["value" => $value, "expire" => $expire,"key"=>$key]));//if you want to store a object or method use set_cache(($key,$value,$expire,$is_serialize=true)
            }
        }
        if($this->driver=="redis")
        {
    ...
    public function get_cache($key,$is_serialize=false,$return_timestamp=false)
    {
        if($this->driver=="file")
        {
            if(($cache=$this->file->read_file($this->path . md5($key) . ".txt"))!=false){
                ...
                if($this->is_expired($cache["expire"])){
                    if($return_timestamp){
                        return ["value"=>$cache["value"],"expire"=>$cache["expire"]];
                    }
                    return $cache["value"];
                }else{
                    $this->delete_key($key);
    ...
    public function increase($key,$value){
        if(($va=$this->get_cache($key,false,true))!=null){
            if(!is_numeric($va["value"])){
                new Exception("400","increase value should be number");
            }
            if($value instanceof \Closure){
                $value=call_user_func($value);
            }
            if(!is_numeric($value)){
                new Exception("400","params value should be number");
            }
            $value=$va["value"]+$value;
            $this->set_cache($key,$value,$va["expire"],false,true);
            return true;
        }
        return false;
    }
